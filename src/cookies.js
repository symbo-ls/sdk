...
//
// Canonical home since 2026-07-28. Previously lived in `@symbo.ls/sdk-bridge`
// — the Supabase multi-project federation layer. These three functions are
// backend-agnostic browser primitives with no federation ties; they were
// only parked in that package. Keeping them there forced `@symbo.ls/sdk`'s
// MAIN entry to statically import a deprecated package through
// `crossAppAuth.js`, which meant importing the SDK at all required a peer
// dependency that nothing installs — sdk's own smoke tests died on
// ERR_MODULE_NOT_FOUND.
//
// `@symbo.ls/sdk-bridge`'s last consumers (workspace `shared/prefs.js`,
// governance `finance-features`) finished migrating to this export and the
// package itself is deleted (2026-07-31) — this is the only copy now.
