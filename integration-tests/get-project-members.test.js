...
function getProjectMembersTestPositive () {
  test('getProjectMembers executed with success', async tape => {
    ...
    tape.ok(response)
    tape.equal(response.projectName, globalProject.name)
    tape.equal(response.projectTier, globalProject.tier)
    tape.equal(response.members.length, globalProject.members.length)
  })
}
