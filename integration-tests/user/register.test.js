...
function regTest (testName, userName) {
  test(`Register ${testName}`, async t => {
    ...
    t.equal(userName, regResponse.user.name, 'Registration successful')
    t.ok(typeof regResponse.user.id === 'string', 'Registration ID is a string')
    const id = regResponse.user.id

    /**
     * Fetch a user
     * @param {string} userId - User ID
     */
    const userStatus = await globalSdk.getUser(id)
    ...
    t.ok(userStatus.memberProjects.length === 0, 'User has no projects')
  })
}
