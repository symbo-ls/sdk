---
name: sdk
description: SDK workspace agent. Owns `@symbo.ls/sdk` — typed HTTP client wrappers for the main server. Single source of truth for every backend call across canvas, workspace, marketplace, preview, etc.
...
`@symbo.ls/sdk-bridge` (the legacy Supabase multi-project federation layer) is
fully retired — both `sdk/packages/sdk-bridge` and `governance/packages/sdk-bridge`
were deleted (sdk `aa01fff`, governance `ae05290`/`fc3bdc0`/`6c42a28`), and the
last two live consumers (`workspace/packages/shared/prefs.js`,
`governance/packages/finance-features`) are gone too (finance-features itself
was retired at governance `6c42a28`). The three cookie helpers
(`readCookie`/`writeCookie`/`parentDomain`) that used to live in sdk-bridge are
now canonical in `sdk/src/cookies.js`, exported from the SDK main entry. Don't
re-introduce a `sdk-bridge` dependency or reference.
...
- `sdk/src/cookies.js` — canonical `parentDomain` / `readCookie` / `writeCookie` (zero deps). Re-exported from `sdk/src/index.js`. `sdk/src/crossAppAuth.js` (parent-domain cookie mirror of localStorage tokens for canvas.symbols.app ↔ us-at.symbols.app SSO) imports from here directly — no cross-package hop.
...
- **`SYMBOLS_APP_USE_MONGO_INTEGRATIONS`** — client-side env flag. Server exposes both paths unconditionally; flag flips which branch of `IntegrationService` hits (server Mongo `/core/org-integrations/**` vs legacy Supabase edge fn — the latter is being phased out with the rest of the Supabase plane).
- **HMAC-gated edge fns are out of SDK scope.** `symbols-admin-ops`, `symbols-user-revoke`, `symbols-email-change` use `s2b` HMAC — server calls them, not client. Don't wrap.
...
6. **localStorage-mocking tests require `Object.defineProperty` for its methods.** If you set methods as plain own properties, `Object.keys(localStorage)` enumerates them and breaks any iteration over stored keys. Methods must be non-enumerable.
...
- **From WORKSPACE-PROJECT**: new handler ops in edge fns (e.g., `calendar.freebusy`, `user.emailChange` families) → SDK wrapper extensions. WORKSPACE-PROJECT writes `🤝 REQUEST → SDK` in their NEEDED file; pick up during session-start sibling scan.
...
- **To MARKETPLACE/WORKSPACE-PROJECT/CANVAS**: they consume the main SDK. Breaking-shape changes need `🤝 REQUEST → <CONSUMER>` + coordination, though per B-102 we don't ship breaking changes anyway.
...
1. **Drift count increased unexpectedly?** `cd server && bun run check-drift` + `git log --oneline --since='15 minutes ago' -- src/core/routes/` → locate the new route + classify (A: intentional server-only, B: out-of-scope territory, C: wrapper candidate).
...
5. **A test fails on `Object.keys(localStorage)` path?** The localStorage mock has methods as enumerable own properties. Fix: `Object.defineProperty(ls, 'getItem', {value, enumerable: false, configurable: true})`.
